...
This standardizes the orientation and trims adaptors in preparation for further processing.
...
For example, we could align reads to a reference transcriptome and define each set of reads mapping to the same transcript as a single group.
This would use the name of the reference sequence as a grouping factor, which can be extracted from BAM files using `r Biocpkg("Rsamtools")`.

A more complex approach would be to perform pairwise alignments between all reads using the `groupReads` function.
This is based on the _minimap2_ aligner and will define groups of reads that have similar sequences (at least 70% sequence identity, by default).
