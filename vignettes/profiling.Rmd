...
This requires both the read sequences^[With adaptors removed, see `?chopReads` in the error correction vignette.]:
...
```{r}
reference # produced by the source'd file.
```
...
... which matches up nicely here, but recall that this is simulated data.
