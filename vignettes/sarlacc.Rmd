...
```{r}
pre.groups <- sarlacc:::groupReads(chopped$reads, truncate=500)
summary(pre.groups)
```

For the sake of speed, we only use the first 500 bp of each read.
...
edist <- expectedDist(aln.out$adaptor1)
hist(edist, xlab="Edit distances", col="grey80")
```

... and we set the threshold to the median edit distanec:

```{r}
(ethresh <- ceiling(median(edist)))
```

We group the UMIs for reads _within_ each level of `pre.groups`:

```{r}
groups <- umiGroup(umis, max.lev1=edit.thresh)
