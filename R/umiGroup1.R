...
{
    if(!is.null(UMI2)){
        UMI1_levenshtein.out <- .umiLevensh(id.filt, UMI1, UMI_length)
        UMI2_levenshtein.out <- .umiLevensh(id.filt, UMI2, UMI_length)
        
        UMI.distance <- .addDistance(UMI1_levenshtein.out, UMI2_levenshtein.out)
        Levensh_threshold <- Levensh_threshold*2
    }else{
        UMI.distance <- .umiLevensh(id.filt, UMI1, UMI_length)
    }
    
    sortedUMI <- vector("list", length(UMI.distance))
    for (i in 1:length(UMI.distance)){
        if (length((which(UMI.distance[[i]]<=Levensh_threshold&sort(table(UMI_filt), decreasing=TRUE)[[i]]>=2*sort(table(UMI_filt), decreasing=TRUE)-1)))!=0){
            sortedUMI[[i]] <- rbind(i, which(UMI.distance[[i]]<=Levensh_threshold&sort(table(UMI_filt), decreasing=TRUE)[[i]]>=2*sort(table(UMI_filt), decreasing=TRUE)-1))
        }else{
            sortedUMI
        }
    }
