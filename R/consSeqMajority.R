consSeqMajority <- function(msalign, cluster.reads, nullborder = 0.6)
...
{
    consseq.c4_maj <- vector("list", length(cluster.reads))
    phredscore_c4_maj <- vector("list", length(cluster.reads))
    read_coverage <- vector("list", length(cluster.reads))
