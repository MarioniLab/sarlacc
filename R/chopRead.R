    ...
    
{
    ...
    chop_read_fwd <- DNAStringSet(str_sub(head(sread(fastqseq))[ind_fwd], end_fwd+1))
    chop_read_rev <- DNAStringSet(str_sub(head(sread(fastqseq))[ind_rev], end=start_rev-1))
    
    chop_qual_fwd <- BStringSet(str_sub(quality(getClass(head(quality(fastqseq))))[ind_fwd], end_fwd+1))
    chop_qual_rev <- BStringSet(str_sub(quality(getClass(head(quality(fastqseq))))[ind_rev], end=start_rev-1))
    ...
    names(chop_read_fwd) <- head(ShortRead::id(fastqseq))[ind_fwd]
    names(chop_read_rev_rev) <- head(ShortRead::id(fastqseq))[ind_rev]
    names(chop_qual_fwd) <- head(ShortRead::id(fastqseq))[ind_fwd]
    names(chop_qual_rev_rev) <- head(ShortRead::id(fastqseq))[ind_rev]
